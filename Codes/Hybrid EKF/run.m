...
function [sochat,socbound,OutModel]=run(ObType,param,Meas,BatType)
...
Data = init(Meas.Voltage(1),Meas.Deformation(1),T,SigmaX0,SigmaV,SigmaW,param,ObType);
...
for k = 1:length(Meas.Time)
 ...
 % Compute state variables according to the algorith mode:
  switch ObType
      case 1
          [sochat(k),socbound(k),OutModel(k),Data] = iter_deformation(dk,ik,Tk,deltat,Data);
      case 2
          [sochat(k),socbound(k),OutModel(k),Data] = iter_voltage(vk,ik,Tk,deltat,Data);
      case 3
          [sochat(k),socbound(k),OutModel(k,:),Data] = iter_hybrid(dk,vk,ik,Tk,deltat,Data);
  end
